...
class GadgetSubfindInputHandler(PynbodyInputHandler):
    ...
    def _get_group_children(self,ts_extension):
        ...
        group_children = {}
        prefix = self._property_prefix_for_type.get(ts_extension,'')
        for i in range(len(h)):
            halo_props = h.get_halo_properties(i,with_unit=False)
            if self._sub_parent_name in halo_props:
                parent = halo_props[self._sub_parent_name]
