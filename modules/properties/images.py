...
class Images(HaloProperties):
    ...
    def calculate(self, halo, properties):
        ...
        g_side, s_side = self.render_gas(halo, size), self.render_stars(halo, size)
        halo.rotate_x(90)
        g_face, s_face = self.render_gas(halo, size), self.render_stars(halo, size)
        halo.rotate_x(-90)
        tx.revert()
        g, s = self.render_gas(halo, size), self.render_stars(halo, size)
