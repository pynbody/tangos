...
import tangos.core.stored_options
...
class PlotController(BaseController):
    ...
    def plotopt(self, id) :
        ...
        try :
            ...
            item = tangos.core.stored_options.ArrayPlotOptions
    ...
    def array(self, id, rel=None) :
        ...
        prop = Session.query(tangos.core.halo_data.HaloProperty).filter_by(id=id).first()
...
#
    ...
    def image_img(self, id) :
        ...
        if halo_id!=prop.halo_id:
            prop = Session.query(tangos.core.halo_data.HaloProperty).filter_by(name_id=name_id, halo_id=halo_id).first()
    ...
    def halo_img(self, id) :
        ...
        props = Session.query(tangos.core.halo_data.HaloProperty).join(tangos.core.halo.Halo).filter(and_(
            tangos.core.halo.Halo.timestep_id == spoint, tangos.core.halo_data.HaloProperty.name_id == prop.name_id)).all()
    ...
    def xy_img(self, id="img") :
        ...
        h = Session.query(tangos.core.halo.Halo).filter_by(id=halo_id).first()
        ts = h.timestep
    ...
    def array_img(self, id, overplot=False, prop=None):
        ...
        with imageThreadLock if not overplot else nocontext :
            if prop is None :
                prop = Session.query(tangos.core.halo_data.HaloProperty).filter_by(id=id).first()
            assert isinstance(prop, tangos.core.halo_data.HaloProperty)
