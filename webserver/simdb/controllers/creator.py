...
class CreatorController(BaseController) :
    def index(self) :
        creators = Session.query(tangos.core.creator.Creator).order_by(tangos.core.creator.Creator.id.desc()).all()

        c.creators = []
        c.breadcrumbs = h.breadcrumbs(tangos.core.creator.Creator)
    ...
    def more(self, id) :
        run = Session.query(tangos.core.creator.Creator).filter_by(id=id).first()
