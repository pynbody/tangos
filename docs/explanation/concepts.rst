...
very few ideas: simulations contain timesteps, timesteps contain objects,
objects carry properties and are joined to one another by links. Merger trees,
black hole histories and comparisons between two runs of the same volume are
all one of those four things under a different name.
...
source, a target, a name and a number. That is the whole idea, and it repays
attention, because tangos uses this one mechanism for *every* relationship it
knows.
...
The most useful consequence: **there is no "main progenitor" flag anywhere in
tangos.** The main progenitor is simply the progenitor link of largest weight.
...
``group``
   ...
   -- SubFind's FoF groups as against its subhalos, for instance.
...
``tracker``
   ...
   finder produced; useful when a finder loses an object you care about.

``phantom``
   A placeholder where a halo is missing from a catalogue but the merger tree
   needs it to stay connected: no particles, no useful properties, a gap.
...
``typetag_number`` -- ``halo_1``, ``BH_3``, ``group_2``, ``phantom_7`` -- and a
bare number means ``halo``, so ``.../1`` and ``.../halo_1`` are the same
object; prefer the explicit form.

The ``%`` surprises people, and is not tangos syntax at all: the timestep is
...
Reach for this when a number looks wrong, or when several people write to one
database: it answers "where did this value come from?" and "what did last
